...
def set_seed(seed):  # noqa: F811  (self-contained)
    random.seed(seed)
    np.random.seed(seed)
...
def run_epoch(model, loader, criterion, device, optimizer=None, desc="train", grad_clip=0.0):
    ...
    return {
        "loss": loss_sum / count, "cls_loss": loss_sum / count,
...
def main():
    ...
    p.add_argument("--data_root", type=str, default=data_paths.DATA_ROOT)
    p.add_argument("--cifar10c_root", type=str, default=data_paths.CIFAR10C_ROOT)
    p.add_argument("--save_dir", type=str, default="./checkpoints_cifar_base")
    ...
    p.add_argument("--resume", type=str, default="", help="path to last_*.pt to resume training")
    ...
    last_path = os.path.join(args.save_dir, f"last_cifar_base_{args.backbone}.pt")
    ...
    if args.resume and os.path.isfile(args.resume):
        ...
        print(f"Resumed from {args.resume} at epoch {start_epoch} (best_val={best_val:.5f})")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        torch.save({"epoch": epoch, "model_state_dict": model.state_dict(),
                    ...
                    "patience_counter": patience_counter, "script_args": vars(args)}, last_path)
    ...
    # CIFAR-10-C OOD evaluation + summary.csv (method='base' for compare.py).
